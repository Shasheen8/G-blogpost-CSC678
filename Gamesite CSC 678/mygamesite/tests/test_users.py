...
from mygamesite.routes import current_user
from flask import request
from mygamesite import bcrypt
from mygamesite.models import User
...
class TestUser(BaseTestCase):
    ...
    def test_user_registeration(self):
        with self.client:
            response = self.client.post('register/', data=dict(
                username='Shashhh', email='shashh@gmail.com',
                password='testing', confirm='testing'
            ), follow_redirects=True)
            self.assertIn(b'Welcome to Flask!', response.data)
            self.assertTrue(current_user.name == "Shashhh")
            self.assertTrue(current_user.is_active())
            user = User.query.filter_by(email='shashh@gamil.com').first()
            self.assertTrue(str(user) == '<name - Shashhh>')

    # Ensure errors are thrown during an incorrect user registration
    def test_incorrect_user_registeration(self):
        with self.client:
            response = self.client.post('register/', data=dict(
                username='Shashhh', email='michael',
    ...
    def test_get_by_id(self):
        with self.client:
            self.client.post('/login', data=dict(
                username="Shashhh", password='testing'
            ), follow_redirects=True)
    ...
    def test_check_password(self):
        user = User.query.filter_by(email='gmail.com').first()
